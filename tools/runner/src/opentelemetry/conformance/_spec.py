...
class ExpectedViolation:
    ...
    that ``id``. That is the right shape when the findings are one gap seen
    many times — an implementation's own attribute namespace, say — and the
    wrong one otherwise, because a declaration that matches a whole class
    stops reporting when the class shrinks. It still fails once the class
    empties, which is the check that keeps suppressions from outliving the gap.
    """
...
class PackageSpec:
    ...
    # Declared once for every scenario. Already merged into each of them as
    # ``inherited_violations``; kept here so the package still reads back.
    expected_violations: tuple[ExpectedViolation, ...] = ()
...
def _parse_scenario(
    ...
    *,
    ...
    # Which reason applies would be anyone's guess, and one of the two is
    # certainly stale. Say so rather than picking.
