...
class Server:
    ...
    def __init__(
        ...
        # The port is reserved by a socket held open until the moment the
        # child is spawned. Binding and closing right away would leave the
        # port free for the whole gap between building a Server and starting
        # it — long enough for a parallel run to take it.
        ...
        # A file rather than a pipe: nothing reads the server's output until
        # something goes wrong, and a pipe nobody drains wedges a chatty
        # server once its buffer fills.
    ...
    def _wait_for_ready(self) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if self._is_ready():
                return
            # Every unready answer waits, not just the ones that raised: a
            # server replying 503 while it warms up would otherwise be polled
            # as fast as it can respond.
